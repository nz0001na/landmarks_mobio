...
preLmk = zeros(38,round(size(C1,1)/5));
...
%     tmpImgO = imread([mainAFLWDir testImg{i}]);
    ...
    csvwrite(['./mobio_result/' name(1:end-3) 'csv'],preLmk(:,i)); 
    timeCost(i) = toc;
    ...
    if verbose
        ...
        a = plot(preLmk(1:end/2,i), preLmk(end/2+1:end,i), 'y*');
%         plot(gtLmk(1:end/2,i), gtLmk(end/2+1:end,i), 'ro');
        saveas(a,['./mobio_img/' name(1:end-3) 'png']);
        hold off;
