...
caffe_root = '/media/guo/SSD/nazhang/caffe/'
...
def show_image(filename, img, facepoint, bboxs, headpose):
    ...
    img = img[:,:,[2,1,0]]
    result_path = '/media/guo/SSD/nazhang/face_landmark/11/face-landmark-localization-master/mobio_result_3/result_img/'
...
def predictImage(img_path):
    ...
    result_path = '/media/guo/SSD/nazhang/face_landmark/11/face-landmark-localization-master/mobio_result_3/'
    if(os.path.exists(result_path) is False):
        os.makedirs(result_path)


    if(os.path.exists(img_path) is False):
        print "Source images path does not exist!"
    else:
        ...
        bbox_list = []
        bbox_list.append(['name','x1','y1','x2','y2','width','height'])

        multi_face_list = []
        multi_face_list.append(['name','faceNum'])
        ...
        for n in range(count):
            ...
            if(faceNum > 1):
                multi_face_list.append([filename, str(faceNum)])
            ...
            for i in range(0, faceNum):
                ...
                width = x2-x1
                height = y2-y1
                item = [filename, x1,y1,x2,y2,width,height]
            ...
            show_image(filename, colorImage, level1Point, bboxs, predictpose)

            points_list = []
            item = ['point', 'x', 'y']
            points_list .append(item)

            for m in range(0, level1Point.shape[0]):
                if m > 0:
                    points_list.append(['','',''])
                for n in range(0,level1Point.shape[1]/2):
                    x = int(round(level1Point[m,n*2]))
                    y = int(round(level1Point[m,n*2+1]))
                    item = [str(n+1), str(x), str(y)]
                    points_list.append(item)

            landmark_path = result_path + 'landmark/'
            if(os.path.exists(landmark_path) is False):
                os.makedirs(landmark_path)
            with open(landmark_path + filename[0:len(filename)-3] + 'csv', 'wb') as p:
                file_p = csv.writer(p)
                file_p.writerows(points_list)


        with open(result_path + 'multi_face_list.csv', 'wb') as r:
            file_t = csv.writer(r)
            file_t.writerows(multi_face_list)


        with open(result_path + 'facial_detection_result.csv', 'wb') as w:
            file_t = csv.writer(w)
            file_t.writerows(bbox_list)
...
if __name__ == '__main__':
    ...
    # image path
    # img_path = '/media/guo/SSD/nazhang/face_landmark/11/face-landmark-localization-master/img/'
    img_path = '/media/guo/SSD/nazhang/face_landmark/mobio_dataset3/'
